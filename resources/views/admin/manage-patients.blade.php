                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <button onclick="toggleStatus({{ $patient->id }})"
                                        class="rounded-lg p-2 text-slate-500 hover:bg-slate-100 hover:text-yellow-600 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-yellow-500"
                                        title="{{ $patient->isActive ? 'Khóa tài khoản' : 'Kích hoạt' }}">
...
<script>
    ...
    function openModal(patientId = null) {
        if (patientId) {
            ...
            fetch(`/quantrivien/manage-patients/${patientId}/edit`)
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        patientForm.action = `/quantrivien/manage-patients/${patientId}`;
    ...
    function toggleStatus(id) {
        if (confirm('Bạn có chắc chắn muốn thay đổi trạng thái tài khoản này?')) {
            fetch(`/quantrivien/manage-patients/${id}/toggle-status`, {
                method: 'POST',
                headers: {
    ...
    function deletePatient(id) {
        if (confirm('Bạn có chắc chắn muốn xóa bệnh nhân này? Hành động này không thể hoàn tác.')) {
            fetch(`/quantrivien/manage-patients/${id}`, {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Content-Type': 'application/json'
